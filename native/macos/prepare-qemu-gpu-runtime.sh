...
usage() {
  cat <<'EOF'
Usage: native/macos/prepare-qemu-gpu-runtime.sh
...
for tool in awk brew codesign curl ditto file install install_name_tool mkdir \
  mktemp otool python3 rm sed shasum sw_vers tar uname; do
...
download_and_verify "QEMU $qemu_version" "$qemu_url" "$qemu_sha256" "$qemu_archive"
download_and_verify "virglrenderer $virgl_version" "$virgl_url" "$virgl_sha256" "$virgl_archive"
download_and_verify "ANGLE $angle_version" "$angle_url" "$angle_sha256" "$angle_archive"
download_and_verify "libepoxy $epoxy_version" "$epoxy_url" "$epoxy_sha256" "$epoxy_archive"
...
replace_required_rpath "$qemu_binary" '@@HOMEBREW_PREFIX@@/lib' '@executable_path/../lib'
