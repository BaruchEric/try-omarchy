...
build_dir="$proof_dir/.build/qemu-8.2-native"
source_overlay="$proof_dir/.build/qemu-8.2-source"
jobs=${BUILD_JOBS:-2}

fail() {
  printf 'PINNED_QEMU_BUILD_FAIL %s\n' "$*" >&2
  exit 1
}
...
if [[ ! -f "$source_overlay/.omarchy-source-commit" ]]; then
  ...
  for name in "${!revisions[@]}"; do
    ...
    git -C "$checkout" archive "${revisions[$name]}" | \
      tar -x -C "$overlay_tmp/subprojects/$name"
    if [[ -d "$source_dir/subprojects/packagefiles/$name" ]]; then
      cp -R "$source_dir/subprojects/packagefiles/$name/." \
        "$overlay_tmp/subprojects/$name/"
    fi
  done
...
docker run --rm --init --platform linux/amd64 \
  --volume "$source_overlay:/qemu-src" \
  --volume "$proof_dir:/proof" \
  --workdir /proof/.build/qemu-8.2-native \
  --env "BUILD_JOBS=$jobs" \
  "$builder_image" \
  bash -lc '
    set -euo pipefail
...
printf 'PINNED_QEMU_BUILD_PASS binary=%s qemu_img=%s\n' \
