...
const KERNEL_COMMAND_LINE = "root=/dev/vda rw rootwait console=tty0 console=ttyS0,115200n8 loglevel=4 systemd.show_status=false rd.systemd.show_status=false mitigations=off nowatchdog omarchy.web_demo=1";
...
export async function verifyGuestArtifacts(guestDirectory) {
  ...
  invariant(manifest.guest?.kernelCommandLine === KERNEL_COMMAND_LINE, "guest manifest kernel command line differs from the proof");
  ...
  for (const artifact of manifest.artifacts) {
    ...
    invariant(information.isFile() && !information.isSymbolicLink(), `artifact is not a regular file: ${relativePath}`);
    invariant(information.size === artifact.bytes, `artifact size mismatch: ${relativePath}`);
  ...
  invariant(spec.runtime?.kernelCommandLine === KERNEL_COMMAND_LINE, "build spec kernel command line differs from the proof");
  invariant(spec.runtime?.minimumMemoryMiB === 1024, "guest minimum memory is not the exact one-vCPU browser allocation");
  invariant(spec.runtime?.recommendedMemoryMiB === 1536, "guest recommended memory is unexpected");
  ...
  return {
    ...
    runtime: {
      minimumMemoryMiB: spec.runtime.minimumMemoryMiB,
      recommendedMemoryMiB: spec.runtime.recommendedMemoryMiB,
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  main().catch((error) => {
    process.stderr.write(`PREBOOT_ARTIFACT_INTEGRITY_FAIL ${error.message}\n`);
    process.exitCode = 1;
  });
}
