...
export async function validateStandaloneQcow2Image(
  filePath,
  { expectedBytes, expectedVirtualBytes, label = "standalone qcow2 image" },
) {
  invariant(
    Number.isSafeInteger(expectedVirtualBytes) && expectedVirtualBytes > 0,
    `expected ${label} virtual size is invalid`,
  );
  const handle = await open(filePath, "r");
  try {
    const info = await handle.stat();
    invariant(info.isFile() && info.size > 0, `${label} must be a non-empty file`);
    if (expectedBytes !== undefined) {
      invariant(info.size === expectedBytes, `${label} byte length changed`);
    }
    const header = await readExact(
      handle,
      QCOW_V3_HEADER_BYTES,
      0,
      `${label} header`,
    );
    invariant(header.readUInt32BE(0) === QCOW_MAGIC, `${label} is not qcow2`);
    invariant(
      header.readUInt32BE(4) === QCOW_VERSION,
      `${label} must use the canonical qcow2 v3 header`,
    );
    invariant(
      header.readBigUInt64BE(8) === 0n && header.readUInt32BE(16) === 0,
      `${label} must not declare a backing file`,
    );
    invariant(
      header.readBigUInt64BE(24) === BigInt(expectedVirtualBytes),
      `${label} virtual size differs from the resume descriptor`,
    );
    invariant(
      header.readUInt32BE(100) >= QCOW_V3_HEADER_BYTES &&
        header.readUInt32BE(100) <= info.size,
      `${label} has an invalid qcow2 header length`,
    );
    return Object.freeze({
      version: QCOW_VERSION,
